...
class PlayFair {
    ...
    string removeDuplicates(string str) {
        string alpha = str + "abcdefghijklmnopqrstuvwxyz0123456789";
    ...
    string decrypt(string cipherText, string key) {
        ...
        for (int i = 0; i < arr.size(); i++) {
            ...
            if (fInd.second == sInd.second) {
                arr[i][0] = mat[mod6(fInd.first - 1)][fInd.second];
                arr[i][1] = mat[mod6(sInd.first - 1)][sInd.second];
            }
            else if (fInd.first == sInd.first) {
                arr[i][0] = mat[fInd.first][mod6(fInd.second - 1)];
                arr[i][1] = mat[sInd.first][mod6(sInd.second - 1)];
            }
            else {
